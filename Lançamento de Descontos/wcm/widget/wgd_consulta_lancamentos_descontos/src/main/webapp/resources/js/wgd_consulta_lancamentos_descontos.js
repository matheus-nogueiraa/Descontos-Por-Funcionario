...
var MyWidget = SuperWidget.extend({
  ...
  setLoading: function (isLoading) {
    if (isLoading) {
      if ($("#switchLoading").length === 0) {
        $("#switchWrapper").append(
          '<span id="switchLoading" class="ml-2"><i class="fluigicon fluigicon-spinner fluigicon-spin"></i></span>',
        );
      }
  ...
  updateSwitchColor: function (isEnabled) {
    ...
    console.log(
      "Cor atualizada - Habilitado:",
      isEnabled,
      "Classe aplicada:",
      isEnabled ? "switch-success" : "switch-danger",
    );
  },
  ...
  consultarParametroInicial: function () {
    ...
    var dataset = DatasetFactory.getDataset(
      "ds_parametro_aprovacao_descontos",
      null,
      constraints,
      null,
    );
  ...
  atualizarParametroAprovacao: function (
    permitirAprovacao,
    documentId,
    cardId,
  ) {
    ...
    $.ajax({
      ...
      .done(function (data) {
        ...
        $("#switchMsg").html(
          '<i class="text-success">✓ Parâmetro atualizado</i>',
        );
...
async function loadTable() {
  ...
  try {
    ...
    if ($.fn.dataTable.isDataTable("#datapanelDescontos")) {
      ...
      $("#datapanelDescontos").addClass(
        "usa-table views-table views-view-table cols-8 sticky-enabled sticky-table",
      );
    ...
    if (dataInclusaoDeFiltro || dataInclusaoAteFiltro) {
      constraints.push(
        DatasetFactory.createConstraint(
          "dataSolicitacao",
          dataInclusaoDeFiltro || "1999-01-01",
          dataInclusaoAteFiltro || "2999-01-01",
          ConstraintType.MUST,
        ),
      );
    }
    ...
    DatasetFactory.getDataset(
      "ds_form_lancamento_desconto_funcionario",
      null,
      constraints,
      null,
      {
        success: function (dataset) {
          ...
          dataset.values.forEach((element) => {
            myData.push([
              ...
              element.nomeColaborador
                ?.split(" - ")
                ?.slice(2)
                ?.join(" - ")
                ?.trim() || "",
              element.grupoAprovadorCC?.split("_")[
                element.grupoAprovadorCC?.split("_")?.length - 1
              ] || "",
              element.codVerba,
...
function consultaDocumentosProcesso(numFluig) {
  ...
  constraintsAnexosProcesso.push(
    DatasetFactory.createConstraint(
      "userSecurityId",
      "admin",
      "admin",
  ...
  const datasetAnexosProcesso = DatasetFactory.getDataset(
    "ds_process_attachments_files",
    null,
    constraintsAnexosProcesso,
    null,
  );
  ...
  if (!datasetAnexosProcesso?.values?.length) {
    FLUIGC.toast({
      title: "Erro: ",
      message:
        "Não foi possível consultar os anexos do processo. Nenhum documento encontrado!",
      type: "warning",
      timeout: "slow",
    });
