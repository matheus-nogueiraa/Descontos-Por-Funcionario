...
var MyWidget = SuperWidget.extend({
    ...
    // Consulta assíncrona ao Dataset 'ds_consultaLancamentoDescontos_frotas'
    buscarDados: function(isInitialLoad) {
        ...
        self.consultarDataset("ds_consultaLancamentoDescontos_frotas", null, constraints, ["numFluig"], {
            success: function(dataset) {
                var rows = [];
                if (dataset) {
                    var rawValues = dataset.values || dataset;
    ...
    inicializarAtividades: function(rows) {
        ...
        if (!v || v.trim() === "" || v.trim() === "null" || v.trim() === "undefined") {
            try {
                var dsFallback = DatasetFactory.getDataset("ds_consultaLancamentoDescontos_frotas", null, null, null);
